...
func Open(dir string) (*Store, error) {
	...
	CREATE TABLE IF NOT EXISTS intake_events (email_id TEXT PRIMARY KEY, webhook_id TEXT NOT NULL UNIQUE, payload BLOB NOT NULL, state TEXT NOT NULL CHECK(state IN ('pending','retry','done','ignored')), attempts INTEGER NOT NULL CHECK(attempts>=0), next_attempt_at INTEGER NOT NULL, queued_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, last_error TEXT NOT NULL);
	CREATE INDEX IF NOT EXISTS intake_due ON intake_events(state,next_attempt_at,queued_at);
